...
|---|---|---|---|
| `01_fetch_fda_warning_letters.py` | Download the fda.gov website warning-letter export (most recent ~1,000 letters; subject lines + letter links) and flag the medical-device subset. Retired as the event source — the dataset and doc were removed 2026-07-06; superseded by script 03. Kept in case letter-level metadata is needed. | Ryan | 🗑 Retired (still runnable) |
| `02_link_warning_letters_to_gvkey.py` | Link website-snapshot letter names to Compustat `gvkey` via WRDS (conservative exact-ish match). Retired with script 01 — superseded by scripts 04–05, which inherit its matching philosophy. | Ryan | 🗑 Retired (still runnable) |

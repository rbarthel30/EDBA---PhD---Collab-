...
| `18_build_combined_filing_event_dataset.py` | **Combined filing x event dataset (`data/combined_9_17_26.csv`).** BASELINE = the full script-17 10-K/10-Q panel (every filing kept, so event vs no-event quarters can be compared). FDA product events and device 8-Ks are each given a **reporting quarter from their own date** - assigned to the filing whose period window contains the date (firm-specific fiscal quarters, 52/53-week calendars and 32 bad EDGAR period dates handled) - then merged on gvkey x reporting year x reporting quarter as counts. Event-level loaders are reconciled EXACTLY against the script-14 panel each run; asserts key uniqueness, event-precedes-filing, and count conservation. Reads MAUDE from script 11's parquet cache (pinned openFDA export). Writes descriptives to `output/tables/` (md) and `meetings/` (pdf). Also computes the panel's **share of US medical-device market cap** (script 09's universe definition recomputed for the 117 panel firms, except that **US exchange listing replaces US incorporation**; WRDS pull cached gitignored in `data/raw/compustat_device_universe_mktcap_*.csv`): 79.9% from 34 of 172 universe firms. First run: 6,547 filings, 117 firms; substantive MD&A mention in 596 (warning letter) / 853 (recall) / 274 (adverse event) filings; 76 event 8-Ks merged. | Ryan | ✅ Working |

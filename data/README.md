...
- **Industry coverage (descriptives only, not a dataset column).** Panel firms' current market cap as a share of the US medical-device universe (US-incorporated, primary SIC 3841-3845, market cap at latest fiscal year-end on/after 2025-06-30): **81.0%**, from 31 panel firms of 167 universe firms. Of the other 86 panel firms, 53 have no current market cap (acquired/delisted), 30 have a non-device SIC, 3 are device firms incorporated abroad (Medtronic, Steris, LivaNova). Four firms (Abbott, Intuitive Surgical, Boston Scientific, Stryker) are 61% of the universe, so the share is driven by them. The WRDS extract is cached at `data/raw/compustat_device_universe_mktcap_2026-09-18.csv` (gitignored, licensed); delete it to re-pull.
